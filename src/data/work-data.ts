// src/data/work-data.ts
export const projects = [
    {
      title: 'Secure Share',
      description: 'A secure platform for sharing files with end-to-end encryption, ensuring privacy and data protection.',
      tags: ['Next.js', 'File Sharing', 'Encryption', 'Security'],
      imageUrl: '/secureShare.png',
      imageHint: 'secure file sharing',
      link: 'https://secure-share-gradgear.vercel.app/',
    },
    {
      title: 'Agri Assist',
      description: 'An intelligent platform to assist farmers with modern agricultural solutions, powered by AI.',
      tags: ['Next.js', 'React', 'AI', 'Agriculture'],
      imageUrl: '/agriAssistImage.png',
      imageHint: 'agriculture technology',
      link: 'https://agri-assist-xi.vercel.app/',
    }
  ];
